...
TokenStream& TokenStream::operator>>(Token*& outputLoc) {
    ...
    if (matchToken("\\s", outputLoc, TokenType::End, true)) return *this;

    if (matchToken("([A-Za-z]+)", outputLoc, TokenType::Identifier, false)) return *this;

    if (matchToken("\\/([^]*?[^\\\\])\\/", outputLoc, TokenType::Regex, false)) return *this;
